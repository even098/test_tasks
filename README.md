...
Создал [канал в телеграм](https://t.me/test_task_4), и загрузил туда заранее подготовленный файл `task4/test.zip`. И его хэш-значение, вычисленный по алгоритму SHA256. Файл содержит `write.exe` и `task4/hash.exe`.
...
2. Запускаем `task4/task4.exe` или task4.py (они не отличаются). Программа на вход может потребовать 
