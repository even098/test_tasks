...
> Важно! Поскольку **subprocess** выполняет команды в сессиях (после выполнения команды сессия сбрасывается), команды по типу `cd`, `set`, `cls`, `exit` выполняются некорректно.
...
Создал [чат в телеграм](https://t.me/test_task_4), и загрузил туда заранее подготовленный файл `task4/test.zip`. И его хэш-значение, вычисленный по алгоритму SHA256. Файл содержит `write.exe` и `task4/hash.exe`.
2. Для получения доступа к сообщениям и файлам чата, зарегистрировал приложение в телеграм и ввел его API Hash, API ID. И авторизовался через номер телефона.
3. Программа `task4/main.exe`.  
...
### Как запустить
1. Регистрируем новое Telegram-приложение по [ссылке](https://my.telegram.org/apps). Требования для полей:
    - App Title: Название вашего приложения. Например: MyBotApp.
    - Short Name: Короткое имя приложения. Оно должно быть уникальным. Например: mybotapp.
    - Platform: Выберите любую платформу, например, Desktop.
    - Description: Краткое описание приложения. Например: Test application for my Telegram bot.
2. Запускаем `task4/task4.exe` или task4.py (они не отличаются). Программа на вход потребует ввода API Hash и API ID приложения, которое было создано, а также номер. Вводим их, авторизуемся (авторизация разовая).
3. После авторизации можно запустить `task4/main.exe` и `task4/hash.exe`.
...
### Шаги выполнения
1. Создание чата Telegram.  
Создал [чат в телеграм](https://t.me/test_task_5).
2. Для отправки сообщений использовал то же приложение, что в Задании 4.
3. Программа `task5/task5.exe`.    
Программа `task2/task2.exe` была написана на языке Python, с использованием различных библиотек для управления "Планировщиком задач Windows", проверки изменения файловой системы (для Windows я выбрал сканировать диск C), архивирования найденных файлов (с паролем) и для выгружения файла в чат, указанный в программе.


### Как запустить
1. Вводим API Hash, API ID созданнаго ранее Telegram-приложения. Если в прошлый раз авторизовывались, еще одна авторизация не потребуется.
2. Запускаем `task5/task5.exe`, либо **task5.py**. Программа отсканирует диск С на наличие созданных/измененных за последние сутки `.txt`, `.xml`, `.xlsx` файлов, архивирует их и выгружает в указанный ранее [Telegram-чат](https://t.me/test_task_5).
...
### Шаги выполнения
1. Программа `task6/task6.exe`.  
Программа была написана на Python, с использованием библиотек для работы с системой, `.zip, .tar, .rar` файлами и библиотекой **requests** для скачивания архива, лежащего на сервере. Она разархивирует скачанный по адресу **%appdata%** файл и запустит лежащие в ней `.exe` файл(ы).


### Как запустить
1. Хостим любой сервер (можно использовать Docker-контейнер из Первого задания), чтобы можно было скачивать архивы по адресу `http://<host_ip>/<archive.ext>`.
2. Запускаем программу или .py файл `task6/task6.exe`. Вводим необходимые данные (айпи, название и расширение архива, пароль).
